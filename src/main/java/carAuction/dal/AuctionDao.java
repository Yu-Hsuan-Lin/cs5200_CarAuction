...
protected ConnectionManager connectionManager;
	...
	public Auction create(Auction auction) throws SQLException {
		...
		try {
			...
			insertStmt.setString(5, auction.getCar().getCarID());
			insertStmt.setString(6, auction.getUser().getUserID());
			...
			insertStmt.setString(12, auction.getCustomerService().getCustomerServiceID());
	...
	public Auction updateCustomerService(Auction auction, CustomerService newCustomerService) throws SQLException {
		...
		try {
			connection = connectionManager.getConnection();
			updateStmt = connection.prepareStatement(updateCar);
			updateStmt.setString(1, newCustomerService.getCustomerServiceID());
			updateStmt.setString(2, auction.getAuctionID());
			updateStmt.executeUpdate();

			// Update the car parameter before returning to the caller.
			auction.setCustomerService(newCustomerService);
			return auction;
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
	...
	public Auction getAuctionById(String auctionID) throws SQLException {
		...
		try {
			...
			if(results.next()) {
				...
				Car car = carDao.getCarById(results.getString("CarID"));
				...
				CustomerService customerService = customerServiceDao.getCustomerServiceById(results.getString("CustomerServiceID"));
				Boolean priceChangeAlert = results.getBoolean("PriceChangeAlert");
				
				Auction auction = new Auction(resultAuctionID,title,startTime,endTime,car,user,highlights,pictures,minimumPrice,currentHighestPrice,auctionStatus,customerService,priceChangeAlert);
				return auction;
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public List<Auction> getAuctionForUser(Users users) throws SQLException {
		...
		try {
			...
			while(results.next()) {
				...
				Auction auction = new Auction(auctionID,title,startTime,endTime,car,user,highlights,pictures,minimumPrice,currentHighestPrice,auctionStatus,customerService,priceChangeAlert);
	...
	public List<Auction> getAuctionForCar(Car car) throws SQLException {
		...
		try {
			...
			while(results.next()) {
				...
				Date startTime =  new Date(results.getTimestamp("StartTime").getTime());
				Date endTime =  new Date(results.getTimestamp("EndTime").getTime());
				Car carByID = carDao.getCarById(results.getString("CarID"));
				Users user = usersDao.getUserFromUserID(results.getString("UserID"));
				String highlights = results.getString("Highlights");
				String pictures = results.getString("Pictures");
				Float minimumPrice = results.getFloat("MinimumPrice");
				...
				Auction auction = new Auction(auctionID,title,startTime,endTime,carByID,user,highlights,pictures,minimumPrice,currentHighestPrice,auctionStatus,customerService,priceChangeAlert);
				auctions.add(auction);
			}
		} catch (SQLException e) {
			e.printStackTrace();
